...
*   **Estimation Modes**:
    *   **Last 6 Months (Heuristics)**: Leverages automated M365 activity reports (`Reports.Read.All`) for instant, high-level tenant estimations without deep scanning.
    *   **Deep Scan (Sampling)**: Performs deep multi-threaded scanning across a customizable percentage of users and teams (e.g., 0.01% to 100%).
